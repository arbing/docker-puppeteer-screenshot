...
RUN sh -c 'echo "deb http://snapshot.debian.org/archive/debian-security/20230711T151208Z bullseye-security main" >> /etc/apt/sources.list' \
    && apt-get -o Acquire::Check-Valid-Until=false update \
    && apt-get install -y --force-yes --no-install-recommends firefox=115.0.1-1 && apt-get clean
